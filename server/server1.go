...
func main() {
	...
	http.HandleFunc("/gif", func(w http.ResponseWriter, r *http.Request) {
		...
		var param = u["cycles"]
		if param == nil {
			fmt.Println("Set default value: 10")
			param = make([]string, 1)
			param[0] = "10"
		}
		c, _ := strconv.ParseFloat(param[0], 64)
		lissajous(w, c)
	})
...
func counter(w http.ResponseWriter, r *http.Request) {
	mu.Lock()
	fmt.Fprintf(w, "Liczba wywołań: %d", count)
	mu.Unlock()
}

var pallette = []color.Color{color.White, color.RGBA{0x00, 0xCC, 0x00, 0xff}, color.RGBA{0xFF, 0x66, 0x66, 0xff}}
